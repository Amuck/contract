...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(BaseApplication.getAppContext().getCacheDir() + "/" + ConstanceUtil.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    // 清除Glide磁盘缓存，自己获取缓存文件夹并删除方法, 主线程中执行
    public boolean cleanCatchDisk() {
        return deleteFolderFile(BaseApplication.getAppContext().getCacheDir() + "/" + ConstanceUtil.GLIDE_CARCH_DIR, true);
    }

    // 清除图片磁盘缓存，调用Glide自带方法， 子线程中执行
